  ...
  .registerWidget(
    ...
    },
    {
      ...
      inputSchema: {
        ...
        selectedProductId: z.string().optional().describe("If provided, generate image with this specific product. Otherwise show product selection."),
        generateImage: z.boolean().optional().describe("If false, only return product suggestions without generating image. Default: false for first call."),
      },
      annotations: {
        readOnlyHint: true,
        openWorldHint: false,
        destructiveHint: false,
      },, selectedProductId, generateImage = false
    },
    async ({ imageUrl, prompt, style, budget }) => {
      try {
        // Step 1: Analyze the room image with Groq (FREE)
        let roomAnalysis;
        if (imageUrl && process.env.GROQ_API_KEY) {
          try {
            roomAnalysis = await analyzeRoomWithGroq(imageUrl);
          } catch (error) {
            console.warn("Room analysis failed:", error);
          }
        }

        // Step 2: Use Groq (FREE) to intelligently match products
        ...
        // Step 3: Search for matching IKEA products
        ...
        // If no image generation requested, return products only for selection
        if (!generateImage && !selectedProductId) {
          const _meta = {
            originalImageUrl: imageUrl,
        // Build prompt with selected product if provided
        let imagePrompt = prompt;
        if (selectedProductId) {
          const selectedProduct = products.find(p => p.id === selectedProductId);
          if (selectedProduct) {
            imagePrompt = `Add ${selectedProduct.name} (${selectedProduct.description}) to the room. ${prompt}`;
          }
        }
        ...
        if (imageUrl && process.env.FAL_API_KEY) {
          ...
          return {
            content: [
              {
                type: "text" as const,
                text: `Found ${products.length} matching IKEA products for: "${prompt}"${style ? ` in ${style} style` : ""}${budget ? ` within €${budget} budget` : ""}. User can now select a product to generate the furnished room image.`,
              },
            ],
            _meta,
          };
        ...
        // Step 4: Generate/edit room image using fal.ai (only when requested)
