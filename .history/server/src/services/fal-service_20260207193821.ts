...
export async function editRoomImage({
  ...
  // Build prompt for furniture addition with actual product
  const enhancedPrompt = [
    productImageUrl ? "Place the product from the reference image into this room" : prompt,
    style ? `${style} style interior design` : "modern interior design",
    "professional architectural photography",
    "realistic furniture placement and perspective",
  ...
  try {
    ...
    const response = await fetch("https://fal.run/fal-ai/flux-pro/kontext/multi", {
      ...
      body: JSON.stringify({
        ...
        strength: 0.75, // How much to change the original image (0-1)
