...
export async function editRoomImage({
  ...
  // Build prompt for multi-image context composition
  const enhancedPrompt = productImageUrl 
    ? [
        ...
        "Position it naturally with correct perspective, scale, and lighting to match the room",
        "Do not change the room itself, only add the new furniture piece",
  ...
  // kontext/multi needs array of images: [room, product]
  const images = productImageUrl ? [imageUrl, productImageUrl] : [imageUrl];

  console.log(`🖼️ Generating with ${images.length} images using kontext/multi:`);
  console.log(`   📍 Image 1 (room context): ${imageUrl}`);
  if (productImageUrl) console.log(`   🛋️ Image 2 (product to extract): ${productImageUrl}`);
  console.log(`   💬 Prompt: ${enhancedPrompt}`);

  try {
    // Use flux-pro/kontext/multi for multi-image context composition
    const response = await fetch("https://fal.run/fal-ai/flux-pro/kontext/multi", {
      ...
      body: JSON.stringify({
        image_urls: images, // [room, product]
