...
function extractProductFromGoogleResult(result: any): IkeaProduct | null {
  try {
    ...
    let imageUrl = 'https://images.unsplash.com/photo-1555041469-a586c61ea9bc?w=400&h=300&fit=crop';
    
    // Priority 1: Use SerpAPI thumbnail if available
    if (result.thumbnail) {
      imageUrl = result.thumbnail;
    }
    // Priority 2: Try IKEA's image URL pattern with article number
    else if (articleNumber && articleNumber !== `${Date.now()}`) {
      // IKEA image pattern: https://www.ikea.com/us/en/images/products/product-name__0XXXXXX_ph.jpg
      // Where XXXXXX is derived from article number
      imageUrl = `https://www.ikea.com/us/en/images/products/${name.toLowerCase().replace(/[^a-z0-9]+/g, '-')}__0${articleNumber.substring(0, 6)}_ph177619.jpg`;
    }
    // Priority 3: Try to extract image from rich snippet
    else if (result.rich_snippet?.top?.extensions) {
      const extensions = result.rich_snippet.top.extensions;
      for (const ext of extensions) {
        if (ext.includes('http') && (ext.includes('.jpg') || ext.includes('.png'))) {
          imageUrl = ext;
          break;
        }
      }
    ...
    console.log(`🖼️ Product: ${name}, Image: ${imageUrl}`);
