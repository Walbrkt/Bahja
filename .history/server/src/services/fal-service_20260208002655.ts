...
async function ensureFalCompatibleUrl(imageUrl: string, falApiKey: string): Promise<string> {
  if (imageUrl.startsWith('data:')) {
    console.log(`🔄 Converting data URI to fal.ai URL...`);
    const matches = imageUrl.match(/^data:([^;]+);base64,(.+)$/);
    if (!matches) throw new Error("Invalid data URI format");
    
    const mimeType = matches[1];
    const buffer = Buffer.from(matches[2], 'base64');
    return await uploadBufferToFal(buffer, mimeType, falApiKey);
  }
  
  if (imageUrl.startsWith('http')) {
    // Download the image and re-upload to fal.ai to ensure it's accessible
    console.log(`🔄 Downloading image from ${imageUrl.substring(0, 80)}... and re-uploading to fal.ai`);
    try {
      ...
      return await uploadBufferToFal(buffer, contentType, falApiKey);
    } catch (err) {
      console.warn(`⚠️ Could not re-upload image, using original URL: ${err}`);
      return imageUrl; // fallback: hope fal.ai can access it directly
    }
  }
