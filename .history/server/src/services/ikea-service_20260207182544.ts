...
Return valid JSON: {"products": [...]}`,
    ...
    console.log("🔍 Groq search for:", query);
    console.log("📦 Groq response:", content);
    
    try {
      const parsed = JSON.parse(content);
      const productsArray = parsed.products || [];
      
      if (!Array.isArray(productsArray) || productsArray.length === 0) {
        console.warn("⚠️ Invalid products array from Groq, falling back to mock");
        throw new Error("Invalid products array from Groq");
      }
      
      console.log(`✅ Found ${productsArray.length} products from Groq`);
      return productsArray.map((p: any) => normalizeProduct(p)).slice(0, limit);
    } catch (error) {
...
/**
 * Normalize product data from Groq response
 */
function normalizeProduct(product: any): IkeaProduct {
  return {
    id: product.id || product.articleNumber || `ikea-${Date.now()}-${Math.random()}`,
    name: product.name || "IKEA Product",
    description: product.description || "",
    price: parseFloat(product.price) || 0,
    currency: "EUR",
    articleNumber: product.articleNumber || "00000000",
    width: product.width || 100,
    depth: product.depth || 50,
    ...
    buyUrl: product.buyUrl || `https://www.ikea.com/se/en/p/product-${product.articleNumber}/`,
