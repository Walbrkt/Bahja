  ...
  .registerWidget(
    ...
    async ({ imageUrl, prompt, style, budget, selectedProductId }) => {
      try {
        ...
        // First, get all products including the selected one
        const products = await searchIkeaProductsAPI({
        ...
        if (!selectedProduct) {
          return {
            content: [{
              type: "text" as const,
              text: `Error: Could not find product with ID ${selectedProductId}. Please try selecting again.`,
            }],
            isError: true,
          };
        }

        console.log(`🎨 Generating with product: ${selectedProduct.name}`);
        console.log(`   🖼️ Product image: ${selectedProduct.imageUrl}`);
        console.log(`   🏠 Room image: ${imageUrl}`);

        const imagePrompt = `Add ${selectedProduct.name} to this room`;
        ...
        if (process.env.FAL_API_KEY) {
          const result = await editRoomImage({
            imageUrl,
            productImageUrl: selectedProduct.imageUrl, // Pass actual product image
            prompt: imagePrompt,
            style,
          });
