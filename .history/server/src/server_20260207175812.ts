...
const server = new McpServer(
  ...
  .registerWidget(
    ...
    },
    {
      ...
      inputSchema: {
        imageUrl: z.string().optional().describe("URL of your room image (provide after selecting a product)"),
        prompt: z.string().optional().describe("Additional furniture description (optional)"),
        ...
        selectedProductId: z.string().optional().describe("Product ID selected from catalogue"),
        generateImage: z.boolean().optional().describe("Set true after selecting product and providing room image"),
      },
      annotations: {
        readOnlyHint: true,
        openWorldHint: false,
    ...
    async ({ imageUrl, prompt, style, budget, selectedProductId, generateImage = false }) => {
      try {
        // PHASE 0: Initial catalogue view (no inputs provided)
        if (!imageUrl && !prompt && !selectedProductId) {
          // Show default IKEA catalogue
          const products = await searchIkeaProductsAPI({
            query: style || "furniture",
            style,
            maxPrice: budget,
            limit: 12,
          });
          ...
          const _meta = {
            prompt && process.env.GROQ_API_KEY) {
          try {
        ...
        const products = await searchIkeaProductsAPI({
          query: aiRecommendations.length > 0 ? aiRecommendations.join(" ") : (prompt || style || "furniture")
            mode: "selection",
          };
          ...
          return {
            content: [
              {
                type: "text" as const,
                text: `Browse ${products.length} IKEA furniture products. Click 'Generate Room' on any product, then provide your room image.`,
              },
            ],
            _meta,
          };
        ...
        if (process.env.GROQ_API_KEY) {
          try {
            ...
            console.warn("AI product matching failed:", error);${prompt ? ` for: "${prompt}"` : ""}${style ? ` (${style} style)` : ""}${budget ? ` within €${budget}` : ""}. Click 'Generate Room' on any product to see it in your space.`,
              },
            ],
            _meta,
          };
        ...
        // PHASE 2: Image Generation (product selected, need room image)
        if (selectedProductId && !imageUrl) {
          // User selected product but hasn't provided room image yet
          const selectedProduct = products.find(p => p.id === selectedProductId);
          return {
            content: [
              {
                type: "text" as const,
                text: `Selected: ${selectedProduct?.name || selectedProductId}. Please provide your room image URL to generate the furnished version.`,
              },
            ],
            isError: false,
          };
        ...
        // PHASE 3: Generate furnished image (has product + room image
          maxPrice: budget,
          limit: 8,
        });
        ...
        if (selectedProductId) {
          const selectedProduct = products.find(p => p.id === selectedProductId);
          if (selectedProduct) {
            imagePrompt = `Add ${selectedProduct.name} (${selectedProduct.description}) to this room
            ],
            _meta,
          };
        }
