...
export default function InteriorArchitect({ furnishedImageUrl, products: initialProducts, originalImageUrl, prompt, style, mode: initialMode = "selection" }: WidgetProps) {
  ...
  useEffect(() => {
    if (toolData?.meta?.products) {
      console.log("Tool returned products:", toolData.meta.products.length);
      setProducts(toolData.meta.products);
      setMode(toolData.meta.mode || "selection");
      setIsLoading(false);
    }
  }, [toolData]);
