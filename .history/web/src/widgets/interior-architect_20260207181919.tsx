...
export default function InteriorArchitect({ furnishedImageUrl, products: initialProducts, originalImageUrl, prompt, style, mode: initialMode = "selection" }: WidgetProps) {
  const [selectedProduct, setSelectedProduct] = useState<IkeaProduct | null>(null);
  const [isLoading, setIsLoading] = useState(false);
  const [products, setProducts] = useState<IkeaProduct[]>(initialProducts || []);
  const [mode, setMode] = useState<"selection" | "result">(initialMode);
  const { callTool, data: toolData } = useCallTool("interior-architect");
  ...
  useEffect(() => {
    if (products.length === 0 && !furnishedImageUrl && !isLoading) {
      setIsLoading(true);
      callTool({});
    }
  }, []);

  // Update products when tool returns data
  useEffect(() => {
    if (toolData?._meta?.products) {
      console.log("Tool returned products:", toolData._meta.products.length);
      setProducts(toolData._meta.products);
      setMode(toolData._meta.mode || "selection");
      setIsLoading(false);
    }
  }, [toolData]);
  ...
  const handleProductSelect = (product: IkeaProduct) => {
    ...
    // Show prompt to user to provide room image
    alert(`Great choice! ${product.name}\n\nNow please call the tool again with:\n- selectedProductId: "${product.id}"\n- imageUrl: your room image URL`);
  };
